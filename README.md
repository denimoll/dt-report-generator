...
* DTRG_SECURE_COOKIES - when `true`, the session cookie is sent only over HTTPS (`Secure` flag). Default `false` so local `http://localhost` dev keeps working. Turn on for any deployment behind TLS.
